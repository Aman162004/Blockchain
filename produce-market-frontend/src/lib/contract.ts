...
// ABI generated from user's contract
export const PRODUCE_MARKET_ABI = [
  ...
  },
  {
    "inputs": [
      ...
      { "internalType": "uint256", "name": "_newPriceInINR", "type": "uint256" }
    ],
  ...
  },
  {
    ...
    "outputs": [
      ...
      { "internalType": "string", "name": "currentStatus", "type": "string" },
      { "internalType": "uint256", "name": "priceinRupees", "type": "uint256" },
      { "internalType": "string", "name": "originFarm", "type": "string" },
      { "internalType": "string", "name": "QRCode", "type": "string" }
    ],
    "stateMutability": "view",
...
export async function getContract(signerRequired = false) {
  ...
  const signer = signerRequired ? await provider.getSigner() : undefined;
  return new Contract(CONTRACT_ADDRESS, PRODUCE_MARKET_ABI, signer ?? provider);
}

export function inrToWei(inr: string) {
  // Treat values as wei-like integers (contract expects rupees unit). No conversion.
  return BigInt(inr);
}

export function formatRupees(value: bigint | number | string) {
  try {
